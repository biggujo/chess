...
public class KnightTestFieldInitializer implements FieldInitializer {
    @Override
    ...
    public List<Piece> initialize(List<Piece> predefinedField) {
        ...
        for (int i = 0; i < Defaults.TILE_AMOUNT; i++) {
            for (int j = 0; j < Defaults.TILE_AMOUNT; j++) {
                ...
                if (i == 4 && j == 3) {
                    newFieldList.add(whiteFactory.getInstance(PieceType.KNIGHT, coordinates));
                    continue;
                }

                if (i == 6 && j == 4) {
                    newFieldList.add(blackFactory.getInstance(PieceType.KNIGHT, coordinates));
                    continue;
                }
